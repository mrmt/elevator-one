...
**最新版: v1.4** — 変更点は [CHANGELOG.md](CHANGELOG.md) を参照。動かしているバージョンはヘッダーのタイトルの右にも出る。配信版ではタグの隣に7桁のコミット番号が付く (スマホ幅では隠れる)。

## 使い方

**最新版は https://elevator-noise.com/ に置いてある。まずはここにアクセスすればよい。**

操作説明は [about.html](about.html) にまとめてある (日本語のあとに英語版)。

手元のファイルで動かす場合は、`index.html` をダブルクリックして Safari か Chrome で開く。ブラウザの自動再生ポリシーにより読み込んだだけでは音が出ないので、XYパッドの中央にある大きな再生ボタン (▶) を押すと鳴りはじめる。このボタンは再生が始まると消え、止めるとまた出る。ボタンの真下には解説ページ `about.html` への入口「あそびかた」(英語表示では `READ ME FIRST`) があり、ボタンと一緒に出し入れされる。ヘッダー右上の再生ボタン (▶) でも鳴りはじめ、隣の一時停止ボタン (⏸) で止まる (スペースキーでもトグルできる)。いまの状態の側が点灯するので、鳴っているかどうかは一目で分かる。▶ と ⏸ はアイコンだけの同じ大きさのボタンで、名称は読み上げ用の `aria-label` とツールチップに入れてある。iOS のロック画面に出るボタンと同じ並び。放置しておくと自動的に変化し続ける。ネットワークには一切接続しない。

iPhone / iPad のような狭い画面では、ヘッダーの下にタブ (雰囲気 / XYパッド / 音作り) が出て、1画面に1セクションずつ表示される。ページ自体はスクロールしない。

ヘッダーのタイトル左にあるアイコンは、ひとつ上の階層 (`../`) へ戻るリンク。

いま鳴っている雰囲気 (mood) の名前は、ヘッダーのバージョンの右・ブラウザのタイトル・ロック画面 / CarPlay の再生情報の3か所に出る。スライダーやXYパッドで値を手で変えたあとは、末尾に ` (edit)` が付く。

ヘッダーの再生ボタンの左にある「解説」(英語表示では `manual`) は、操作説明のページ `about.html` へのリンク。表示言語に合わせて日本語版 / 英語版の位置へ飛ぶ。

### VJ ウィンドウ (`?vjw`)

URL の末尾に `?vjw` を付けて開く (例: `https://elevator-noise.com/one/?vjw`) と、本体とは別に VJ ウィンドウが開く。真っ黒な背景に等幅フォントで、全パラメータの値、ドローン各声やフィルタの実際の周波数・ゲイン、シーケンサのパターン、コード進行、スペクトラム、波形の16進ダンプ、発音イベントのログなどが毎フレーム書き換わり続ける。VDMX などの VJ ソフトでこのウィンドウをキャプチャして素材に使う想定。音と操作は従来どおり本体のタブで行う。

ブラウザのポップアップブロックで読み込み時に開けなかった場合は、再生ボタンを押した時点で開く。閉じてしまったら、ヘッダーの「解説」の右に出る `VJW` で開き直せる。VJ ウィンドウの中身は本体が描いているので、本体のタブを閉じる / リロードすると一緒に閉じる。
