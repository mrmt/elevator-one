...
URL の末尾に `?vjw` を付けて開く (例: `https://elevator-noise.com/?vjw`) と、本体とは別に VJ ウィンドウが開く。真っ黒な背景に等幅フォントで、全パラメータの値、ドローン各声やフィルタの実際の周波数・ゲイン、シーケンサのパターン、コード進行、スペクトラム、波形の16進ダンプ、発音イベントのログなどが毎フレーム書き換わり続ける。VDMX などの VJ ソフトでこのウィンドウをキャプチャして素材に使う想定。音と操作は従来どおり本体のタブで行う。
