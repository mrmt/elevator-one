...
ただし右上の小さなボタンだけでは、**まず押さないと音が出ない**ことに利用者が気づけない
（Issue #36）。そこで XYパッドの中央に、YouTube と同じ位置・形の大きな円形ボタン
`.bigplay` を重ねてある。`setPowerUI(on)` が `hidden` を切り替えるので、再生が始まると
消えてパッドの邪魔をせず、止めればまた出る。パッドの上に載っているため `pointerdown` の
伝播を止めてあり、押しただけで目標（bright / density）が動くことはない。
大きさはパッドを基準にした `clamp(4.2rem, 26cqmin, 9rem)` で、狭幅でも縦長でも中央に収まる。

ヘッダーの並びは次の4つで役割を分けてある。

- **ホームリンク**（`a.home`）— タイトル左のアイコン。`href="../"` で親ディレクトリへ戻る。
  単一ファイル構成を保つため SVG は直書きし (色はタイトルと同じ `--ink`)、文字を持たないので `align-self:center` で行の中央に置く
...
- デプロイ (`mrmt/elevator-noise` の `site/Makefile`、このリポジトリの外) は
  `git -C ~/agent/elevator-one -c core.abbrev=7 archive HEAD index.html | tar -x -C public/one/` で取り出す。
