...
- **例外は録音ボタンだけ** — 「録音 / 停止 / 保存 / 録音不可」は状態でラベルが変わるので
  `updateRecLabel()` が持つ。`applyI18n()` から呼ばれる（再生 / 一時停止は2つのボタンに
  分けたのでラベルが状態で変わらず、`data-i18n` で足りる）
...
- **再生 / 一時停止 / 録音**（`.tbtn.xport`）— それぞれ ▶ / ⏸ / ● のアイコンを持つ操作ボタン。
  狭幅（900px以下）では再生と一時停止のラベルだけ `display:none` にしてアイコンに畳み、
  ヘッダーの1行を守る（`aria-label` は残るので読み上げは変わらない）。録音はラベルで
  状態（「停止 / 保存」「録音不可」）を伝えるため文字を残す
- **言語切替**（`.sub.lang`）— 操作ボタンではなく表示の切り替えなので、枠を持たせず
  バージョン表示と同じ `.sub` の字面でその隣に置く。押せることは hover と focus で示す

録音ボタンはラベルだけが状態で変わるので、`updateRecLabel()` は `textContent` ではなく
`.tlabel` を差し替える（ボタン直下の `textContent` を書くと ● の `<svg>` ごと消える）。
